...
def test_stageD():
    ...
    print("language_consistency_reward PASS ✓")

    # ── Test combined reward ──────────────────────────────────────
    from slm_project.config import GRPOConfig
    gcfg = GRPOConfig()

    # Perfect response: correct boxed answer + proper think tags
    ...
    assert perfect_r > 0.8, f"Perfect response should score > 0.8, got {perfect_r}"
    ...
    assert wrong_r < 0.4, f"Wrong answer should score < 0.4, got {wrong_r}"

    print("compute_combined_reward PASS ✓")
    print("STAGE D PASSED ✓  (All reward functions verified)")
